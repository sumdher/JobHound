...
BACKUP_SCRIPT_SOURCE="${SCRIPT_DIR}/jobhound-db-backup.sh"
SERVICE_SOURCE="${SCRIPT_DIR}/jobhound-db-backup.service"
TIMER_SOURCE="${SCRIPT_DIR}/jobhound-db-backup.timer"
COMPOSE_FILE="${REPO_ROOT}/docker-compose.prod.yml"

SYSTEMD_DIR="/etc/systemd/system"
SERVICE_NAME="jobhound-db-backup.service"
TIMER_NAME="jobhound-db-backup.timer"
BACKUP_DIR="/var/backups/jobhound"
...
require_root() {
  [[ "${EUID}" -eq 0 ]] || fail "Run this installer as root (for example: sudo ./deploy/backup/install-jobhound-db-backup.sh)"
}
...
Environment=JOBHOUND_BACKUP_DIR=${BACKUP_DIR}
Environment=JOBHOUND_BACKUP_RETENTION_DAYS=14
Environment=JOBHOUND_BACKUP_COMPOSE_FILE=${COMPOSE_FILE}
Environment=JOBHOUND_BACKUP_PROJECT_DIR=${REPO_ROOT}
Environment=JOBHOUND_BACKUP_SERVICE_NAME=db
EOF
...
log "Installing JobHound database backup systemd units"
