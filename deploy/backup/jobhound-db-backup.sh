...
COMPOSE_FILE="${JOBHOUND_BACKUP_COMPOSE_FILE:-${REPO_ROOT}/docker-compose.prod.yml}"
COMPOSE_PROJECT_DIR="${JOBHOUND_BACKUP_PROJECT_DIR:-${REPO_ROOT}}"
BACKUP_DIR="${JOBHOUND_BACKUP_DIR:-/var/backups/jobhound}"
RETENTION_DAYS="${JOBHOUND_BACKUP_RETENTION_DAYS:-14}"
SERVICE_NAME="${JOBHOUND_BACKUP_SERVICE_NAME:-db}"
LOCK_FILE="${JOBHOUND_BACKUP_LOCK_FILE:-/var/lock/jobhound-db-backup.lock}"
...
[[ "${RETENTION_DAYS}" =~ ^[0-9]+$ ]] || fail "JOBHOUND_BACKUP_RETENTION_DAYS must be a non-negative integer"
...
FILE_BASENAME="jobhound-db-${HOSTNAME_SHORT}-${POSTGRES_DB}-${TIMESTAMP}.dump"
...
if (( RETENTION_DAYS > 0 )); then
  ...
  find "${BACKUP_DIR}" -maxdepth 1 -type f \( -name 'jobhound-db-*.dump.gz' -o -name 'jobhound-db-*.dump.gz.sha256' \) -mtime +"${RETENTION_DAYS}" -print -delete
else
  log "Retention pruning disabled because JOBHOUND_BACKUP_RETENTION_DAYS=${RETENTION_DAYS}"
fi
